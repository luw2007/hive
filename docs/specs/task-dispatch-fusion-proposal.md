...
## 3. 独立 Agent 驱动：CTO + 董秘

### 概念

| 角色 | 视角 | 职责 |
|---|---|---|
| **CTO Agent** | 全局（跨 workspace） | 规划优先级、技术决策、资源分配 |
...
| 维度 | CTO | 董秘（推荐先做） |
|---|---|---|
| 生命周期 | 全局 singleton agent | 每 workspace 一个（类 Orch 的辅助 agent） |
| 数据访问 | 跨 workspace tasks + dispatches | 当前 workspace 的 tasks + dispatches + worker 状态 |
| UI 入口 | 主页 sidebar 顶部 / 独立面板 | 右下角 chat bubble（类 intercom widget） |
| 与 Orch 关系 | 高于 Orch（可指导） | 辅助 Orch（只建议，不替代） |
| 实现复杂度 | 高（跨 workspace 权限、全局 agent 生命周期） | 中（特殊角色 PTY + 独立 UI 面板） |
...
CTO 留后期，需跨 workspace query + 全局 agent lifecycle 基建。
...
| 独立 Agent | 先做董秘（workspace 内） | 跳过，仅增强 Orch prompt | 直接做 CTO |
...
4. **P3**：CTO agent（跨 workspace 基建就绪后）
